...
\pnum
如果条件为表达式$e$\footnote{因为赋值表达式的类型是\tcode{void}，形如\tcode{p = e}的程序文本将始终被看做一个模式匹配而不是赋值表达式。}，那么这个表达式的类型必须实现\tcode{core.Boolean}。条件成立当且仅当$e$求值为真。如果条件是绑定，那条件成立当且仅当绑定成功。该绑定必须可以失败。
