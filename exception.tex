...
\pnum
如果一个异常传播到了最顶层函数的最外层，将会调用\tcode{core.panic}。

\pnum
\tcode{catch}表达式捕获异常并将其包装到\tcode{core.Result}中。表达式的类型为\tcode{Result<T, E>}，其中\tcode{T}为表达式的类型，\tcode{E}为异常的类型。如果表达式$e$抛出异常$e^\dagger$，则其值为$\tcode{.Error(}e^\dagger\tcode{)}$，否则其值为$\tcode{.OK(}e\tcode{)}$。
