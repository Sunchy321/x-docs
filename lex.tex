...
\topline
\tcode{_}         & \tcode{any}       & \tcode{async}     & \tcode{await}     & \tcode{auto}      \\
\tcode{bool}      & \tcode{break}     & \tcode{catch}     & \tcode{class}     & \tcode{cmp}       \\
\tcode{const}     & \tcode{continue}  & \tcode{deinit}    & \tcode{defer}     & \tcode{do}        \\
\tcode{dyn}       & \tcode{else}      & \tcode{enum}      & \tcode{extern}    & \tcode{false}     \\
\tcode{finally}   & \tcode{float}     & \tcode{for}       & \tcode{func}      & \tcode{if}        \\
...
\tcode{operator}  & \tcode{partial}   & \tcode{private}   & \tcode{public}    & \tcode{return}    \\
\tcode{self}      & \tcode{shl}       & \tcode{shl_eq}    & \tcode{shr}       & \tcode{shr_eq}    \\
\tcode{some}      & \tcode{static}    & \tcode{string}    & \tcode{this}      & \tcode{throw}     \\
\tcode{true}      & \tcode{try}       & \tcode{type}      & \tcode{typeof}    & \tcode{uint}      \\
...
\begin{floattable}{多字符标点符号}{tab:multichar-punc}{llllll}
\topline
\tcode{\{|} &
\tcode{|\}} &
\tcode{->}  &
...
\pnum
整数字面量由一系列数字构成。其中的下划线用作分隔并且不影响字面量的值。字面量的前缀用于指示它的进制。\term{十进制字面量}由若干十进制数字构成；\term{十六进制字面量}由前缀\tcode{0x}后跟若干十六进制数字构成；\term{二进制字面量}前缀\tcode{0b}后跟若干二进制数字构成。\X 不支持八进制字面量。
...
\begin{bnf}{StringLiteral}
    ...
    \terminal{@}\bnfp \terminal{"} Rchar\bnfs \terminal{"} \terminal{@}\bnfp Suffix\bnfq \br
...
\pnum
字符串字面量表示UTF-8字符串，其类型为\tcode{string}。它有两种类型：普通字符串字面量和多行字符串字面量。

\pnum
普通字符串字面量被单个双引号包围。其中可以使用反斜杠开始的转义序列表示其他字符。普通字符串字面量也可以前后加上等量的\tcode{@}，此时它被称为\term{原始字符串字面量}。原始字符串字面量中的反斜杠不会被解释为转义序列，而是字符本身。
