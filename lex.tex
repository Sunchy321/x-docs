...
\tcode{await}     & \tcode{auto}      & \tcode{bool}      & \tcode{break}     & \tcode{catch}     \\
\tcode{char}      & \tcode{class}     & \tcode{cmp}       & \tcode{const}     & \tcode{continue}  \\
\tcode{defer}     & \tcode{do}        & \tcode{dyn}       & \tcode{else}      & \tcode{enum}      \\
\tcode{extern}    & \tcode{false}     & \tcode{float}     & \tcode{for}       & \tcode{func}      \\
\tcode{if}        & \tcode{impl}      & \tcode{import}    & \tcode{in}        & \tcode{infer}     \\
\tcode{init}      & \tcode{int}       & \tcode{internal}  & \tcode{is}        & \tcode{let}       \\
\tcode{macro}     & \tcode{match}     & \tcode{module}    & \tcode{mut}       & \tcode{never}     \\
\tcode{nil}       & \tcode{operator}  & \tcode{partial}   & \tcode{private}   & \tcode{public}    \\
\tcode{ref}       & \tcode{return}    & \tcode{self}      & \tcode{shl}       & \tcode{shl_eq}    \\
\tcode{shr}       & \tcode{shr_eq}    & \tcode{some}      & \tcode{static}    & \tcode{string}    \\
\tcode{this}      & \tcode{throw}     & \tcode{trait}     & \tcode{true}      & \tcode{try}       \\
\tcode{type}      & \tcode{typeof}    & \tcode{uint}      & \tcode{unsafe}    & \tcode{void}      \\
\tcode{while}     &                   &                   &                   &                   \\
\end{floattable}
