...
\tcode{bool}      & \tcode{break}     & \tcode{class}     & \tcode{cmp}       & \tcode{const}     \\
\tcode{continue}  & \tcode{deinit}    & \tcode{do}        & \tcode{dyn}       & \tcode{else}      \\
\tcode{enum}      & \tcode{extern}    & \tcode{false}     & \tcode{float}     & \tcode{for}       \\
\tcode{func}      & \tcode{if}        & \tcode{impl}      & \tcode{import}    & \tcode{in}        \\
\tcode{init}      & \tcode{infer}     & \tcode{int}       & \tcode{internal}  & \tcode{is}        \\
\tcode{lazy}      & \tcode{let}       & \tcode{match}     & \tcode{mut}       & \tcode{never}     \\
\tcode{nil}       & \tcode{operator}  & \tcode{partial}   & \tcode{private}   & \tcode{public}    \\
\tcode{return}    & \tcode{self}      & \tcode{shl}       & \tcode{shl_eq}    & \tcode{shr}       \\
\tcode{shr_eq}    & \tcode{some}      & \tcode{static}    & \tcode{string}    & \tcode{this}      \\
\tcode{throw}     & \tcode{true}      & \tcode{type}      & \tcode{typeof}    & \tcode{uint}      \\
