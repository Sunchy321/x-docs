...
\pnum
整数字面量由一系列数字构成。其中的单引号用作分隔并且不影响字面量的值。字面量的前缀用于指示它的进制。最左边的数字具有最大的权重值。\term{十进制字面量}由若干十进制数字构成；\term{十六进制字面量}由前缀 \tcode{0x} 后跟若干大写十六进制数字构成；\term{二进制字面量}前缀 \tcode{0b} 后跟若干二进制数字构成。\footnote{前缀中的字母不能为大写字母；十六进制数字 A - F 不能为小写字母。}
...
\rSec2[literal.character]{字符字面量}

\begin{bnf}
\nontermdef{CharacterLiteral} \br
...
\begin{bnf}
\nontermdef{SimpleEscapeSeq} \textnormal{one of} \br
    \terminal{\textbackslash{}0 \textbackslash{}n \textbackslash{}t \textbackslash{}r \textbackslash\textquotesingle\ \textbackslash\textquotedbl}
\end{bnf}

\begin{bnf}
\nontermdef{UnicodeEscapeSeq} \br
    \terminal{\textbackslash{}u\{} HexadecimalDigitSeq \terminal{\}}
\end{bnf}

\begin{bnf}
\nontermdef{HexadecimalDigitSeq} \br
    HexadecimalDigit \br
