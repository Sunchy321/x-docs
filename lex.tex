...
\tcode{extern}    & \tcode{false}     & \tcode{finally}   & \tcode{float}     & \tcode{for}       \\
\tcode{func}      & \tcode{if}        & \tcode{impl}      & \tcode{import}    & \tcode{in}        \\
\tcode{init}      & \tcode{infer}     & \tcode{int}       & \tcode{internal}  & \tcode{is}        \\
\tcode{lazy}      & \tcode{let}       & \tcode{macro}     & \tcode{match}     & \tcode{mut}       \\
\tcode{never}     & \tcode{nil}       & \tcode{operator}  & \tcode{partial}   & \tcode{private}   \\
\tcode{public}    & \tcode{return}    & \tcode{self}      & \tcode{shl}       & \tcode{shl_eq}    \\
\tcode{shr}       & \tcode{shr_eq}    & \tcode{some}      & \tcode{static}    & \tcode{string}    \\
\tcode{this}      & \tcode{throw}     & \tcode{true}      & \tcode{try}       & \tcode{type}      \\
\tcode{typeof}    & \tcode{uint}      & \tcode{void}      & \tcode{while}     &                   \\
\end{floattable}
...
普通字符串字面量被单个双引号包围。其中可以使用反斜杠开始的转义序列表示其他字符。
