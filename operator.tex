...
\pnum
\tcode{Failable::unwrap}需要返回一个\tcode{ControlFlow}类型。如果返回\tcode{.Return($e$)}，则将这个值作为后缀\tcode{!}的值；如果返回\tcode{.Throw($e$)}，则将这个值作为异常抛出。
...
\pnum
\tcode{in}运算符与\tcode{Include::include}关联。与其他二元运算符相反的是，$a\mathbin{\tcode{in}}b$将被翻译为\tcode{$b$.include($a$)}。
