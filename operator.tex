...
\rSec2[op.over.inc-dec]{自增自减运算符}
\indextext{运算符!重载!自增自减}

\pnum
自增自减运算符的返回值类型必须为\tcode{void}或\tcode{never}。
...
\enternote 虽然\tcode{!in}不能被重载，但是它也有类似的语义。\exitnote

\pnum
一些类型会自动生成默认的\tcode{operator==}运算符：
...
仅当其中用到的\tcode{operator==}存在时，该运算符才会被生成。
