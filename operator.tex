...
二元\tcode{+} & 二元\tcode{-} & \tcode{shl} & \tcode{shr} & \tcode{'\&} \\
\tcode{'\^{}} & \tcode{'|} & \tcode{..} & \tcode{..=} & \tcode{\~} \\
\tcode{==} & \tcode{!=} & \tcode{<} & \tcode{<=} & \tcode{>} \\
\tcode{>=} & \tcode{!<} & \tcode{!>} & \tcode{<>} & \tcode{cmp} \\
\tcode{in} & \tcode{\&} & \tcode{|} &&\\
\hhline{|=====|}
\tcode{.} & \tcode{await} & 后缀\tcode{!} & \tcode{??} & \tcode{!in} \\
...
\pnum
自定义的运算符不能与内建的运算符或字符字面量相同，也不能是\tcode{'}、\tcode{''}、\tcode{//}、\tcode{/*}、\tcode{->}、\tcode{=>}或\tcode{...}。如果自定义的运算符是一个标识符，则它不能在任何作用域中与另一个标识符相同，否则这是一个编译错误。自定义的运算符也不能是\tcode{prefix}、\tcode{suffix}或\tcode{infix}。
...
\pnum
对于重载了的可空运算符调用$e\tcode{?}$或$e\tcode{!}$而言，将首先连续调用\tcode{operator suffix?}直到得到一个可空类型，然后对其进行内建的可空运算符求值。
