...
\pnum
可重载的内建运算符也都有其对应的概念方法。所有概念方法都位于模块\tcode{core.ops}中。
...
\pnum
下标运算符对应四个概念方法：\tcode{Index::index}、\tcode{IndexAssign::indexAssign}、\tcode{IndexRef::indexRef}和\tcode{IndexRefMut::indexRefMut}。这四个概念都需要一个函数类型的泛型参数。
...
\pnum
函数调用运算符有三个对应的概念方法：\tcode{FunctorMut::callMut}、\tcode{Functor::call}和\tcode{FunctorOnce::callOnce}，分别对应可变、不可变和只能调用一次的情况。这三个概念都需要一个函数类型的泛型参数。
...
\begin{floattable}{中缀运算符概念}{tab:trait-infix}{ll}
\topline
运算符 & 概念 \\
\capsep
