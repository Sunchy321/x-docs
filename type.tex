...
\begin{aligned}
    ...
    T_{l,h} &\preceq \tcode{float<}s\tcode{>} \\
...
\pnum
整数类型是浮点类型的子类型。当整数被转换为浮点数时，其值将被转换为最接近的浮点数。\enternote 虽然整数转换到浮点数可能无法保持值不变，但出于习惯仍然保持这个隐式转换。 \exitnote \enternote 浮点类型不能转换到整数类型。请使用转换函数。 \exitnote

\pnum
\tcode{bool}没有语言内建的子类型约束。但是，其他类型的值可以在\tcode{if}表达式中当作调教使用，这通过重载\tcode{operator if}运算符实现。
