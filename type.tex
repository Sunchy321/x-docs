...
\pnum
\tcode{float<$s$>}称作\term{浮点类型}。其中$s$为32或64。\tcode{float}为\tcode{float<64>}的别名。
...
\pnum
\tcode{$T$?}为\term{可空类型}。\tcode{$T$?}包含$T$的所有值（使用\tcode{some}标识）和空值\tcode{nil}。
...
\pnum
结果类型\tcode{$T$ throw $E$}标识一个可能产生错误的值，其中$T$是正常的返回值类型，$E$是错误类型，且必须实现\tcode{ErrorCode}。
