...
\pnum
$T\tcode{[]}$为\term{数组类型}，代表若干个$T$的值的序列。
...
\pnum
结果类型$T\tcode{ throw }E$标识一个可能产生错误的值，其中$T$是正常的返回值类型，$E$是错误类型，且必须实现\tcode{ErrorCode}。$T$和$E$都不能是结果类型。
