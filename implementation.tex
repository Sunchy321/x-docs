...
\pnum
析构器是名称为关键字\tcode{deinit}的方法。
析构器不接受任何非\tcode{this}参数且不返回值。析构器不能抛出异常。
只能为不透明类型的实现提供析构器。

\pnum
析构器对应的特征为\tcode{core::Deinit}。
不能在\tcode{core::Deinit}之外的特征中包含析构器。
不能在\tcode{core::Deinit}之外的实现中包含析构器。
