...
\begin{bnf}{CatchExpr}
    SuffixExpr \terminal{.} \terminal{catch}
\end{bnf}
...
\rSec2[expr.try]{\tcode{try}运算符}
\indextext{运算符!\idxcode{try}}

\begin{bnf}{TryExpr}
    \terminal{try} SuffixExpr
\end{bnf}

\pnum
\tcode{try}表达式用于显式使用一个可能会抛出异常的表达式。如果其操作数不能抛出异常，这是一个编译错误。
