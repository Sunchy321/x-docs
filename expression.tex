...
\pnum
如果$T$是对象类型，则每个$kv$对都会用来初始化对应的成员。如果$v$不能隐式转换到成员的类型，则这是一个编译错误。如果有成员未被显式指定初始值，且其不是可空类型，则这是一个编译错误；如果是可空类型，则其初始值为\tcode{nil}。
...
\begin{bnf}{DeductedEnumerator}
    ...
    \terminal{.} Identifier \terminal{\{} ObjectItems\bnfq \terminal{\}}
\end{bnf}
...
如果$s$实现了\tcode{core.Sequence}（例如内建数组$T\tcode{[]}$），其值为$s\tcode{.size}$。
...
\begin{bnf}{IfExpr}
    ...
    \terminal{if} Condition \terminal{then} Expression \terminal{else} IfStatement
\end{bnf}
...
\begin{bnf}{DoExpr}
    LambdaQual\bnfs \terminal{do} Block \br
    LambdaQual\bnfs \terminal{do} \terminal{!} Block
\end{bnf}
...
\pnum
\tcode{do!}后跟一个块创建一个无参的lambda表达式并立即调用它，将其值作为整个表达式的值。
...
\pnum
\tcode{await}运算符用于等待一个异步表达式的结果。$e\tcode{.await}$挂起当前计算，直到$e$的值可用。如果$e$的类型是$\tcode{core.Future<}T\tcode{>}$，则$e\tcode{.await}$的类型是$T$。只能在具有\tcode{async}修饰的函数作用域中使用\tcode{await}运算符。
...
\rSec2[expr.catch]{\tcode{catch}表达式}
\indextext{表达式!\idxcode{catch}}

\begin{bnf}{CatchExpr}
    SuffixExpr \terminal{.} \terminal{catch} \br
    SuffixExpr \terminal{.} \terminal{?} \terminal{catch}  \br
    SuffixExpr \terminal{.} \terminal{!} \terminal{catch}
\end{bnf}

\pnum
\tcode{.catch}捕获表达式的异常，被捕获的表达式$e$必须是能抛出异常的表达式。结果的类型为$\tcode{core.Result<}T\tcode{>}$，其中$T$为$e$的类型。参见~\ref{core.result}。

\pnum
\tcode{.?catch}的类型是$T\tcode{?}$，如果被捕获表达式抛出异常，则结果为\tcode{nil}，否则结果为被捕获的表达式。

\pnum
\tcode{.!catch}的类型是$T$，如果被捕获表达式抛出异常，则调用\tcode{core.panic}。参见~\ref{core.panic}。
...
\pnum
运算符\tcode{..}生成左闭右开区间，结果类型是\tcode{core.Range}。
运算符\tcode{..=}生成左闭右闭区间，结果类型是\tcode{core.\allowbreak{}ClosedRange}。
...
\pnum
$a\mathbin{\tcode{??}}b$首先对$a$求值，如果其结果不是\tcode{nil}，则表达式的值为$a$，且$b$不会被求值；否则表达式的值为$b$。

\rSec1[expr.cmp-in]{比较运算符、包含运算符和匹配运算符}
...
\pnum
$a \mathrel{\tcode{is}} p$检测$a$是否匹配模式$p$。$a \mathrel{\tcode{!is}} p$等价于$\tcode{!(}a \mathrel{\tcode{is}} p\tcode{)}$。表达式的类型为\tcode{bool}。使用的模式不能包含绑定模式。
