...
\pnum
一些表达式为\tcode{位置表达式}，表示逻辑上的内存位置。这包括：
\begin{itemize}
    \item 变量名称，包括局部变量和全局变量；
    \item \tcode{this}；
    \item 前缀\tcode{*}运算符的结果；
    \item 下标运算符\tcode{[]}的结果；
    \item 成员访问运算符\tcode{.}的结果；
    \item 操作数是位置表达式的后缀\tcode{?}及\tcode{!}表达式；
    \item 内部是位置表达式的括号表达式。
\end{itemize}
...
左操作数称为\tcode{赋值目标表达式}，并递归定义为：
\begin{itemize}
    \item 一个位置表达式，或
    \item \tcode{_}，或
    \item \tcode{...}，或
    \item \tcode{...}后跟赋值目标表达式，或
    \item 成员全部都为赋值目标表达式的结构字面量，或
    \item 成员全部都为赋值目标表达式的数组字面量。
\end{itemize}

左操作数包含的所有位置表达式都必须是\tcode{mut}的。
如果左操作数并非单个位置表达式，则使用类似模式匹配的规则进行赋值。

\pnum
复合赋值运算符\tcode{+=}、\tcode{-=}、\tcode{*=}、\tcode{/=}、\tcode{\%=}、\tcode{shl_eq}、\tcode{shr_eq}、\tcode{'\&=}、\tcode{'\^{}=}、\tcode{'|=}、\tcode{\~=}和\tcode{??=}是对应二元运算符的赋值简化。
