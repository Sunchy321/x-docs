...
\pnum
如果$T$是对象类型，则每个$kv$对都会用来初始化对应的成员。如果$v$不能转换到成员的类型，则这是一个编译错误。如果有成员未被显式指定初始值，且其不是可空类型，则这是一个编译错误；如果是可空类型，则其初始值为\tcode{nil}。
...
\pnum
\tcode{=}将左操作数的值更新为右操作数的值。左操作数必须是\tcode{mut}的，且右操作数必须能转换到左操作数。
