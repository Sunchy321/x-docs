...
\pnum
元组字面量\tcode{($e_1$, $\ldots$, $e_n$)}表示一个显式写出其各元素的元组值。
其类型为\tcode{($T_1$, $\ldots$, $T_n$)}，其中$T_i$为$e_i$的类型。
如果其中包含形如\tcode{...$e$}的项，则视同将$e$的各元素显式插入在该位置。$e$必须也是一个元组。
特别的，\tcode{($e$,)}表示一元元组，此时逗号不能省略。\tcode{()}的类型为\tcode{void}，是\tcode{void}的唯一值。

\pnum
结构字面量\tcode{\{$k_1$:$e_1$, $\ldots$, $x_n$:$e_n$\}}表示一个显式写出其各元素的结构值。
其类型为\tcode{\{$k_1$:$T_1$, $\ldots$, $x_n$:$T_n$\}}，其中$T_i$为$e_i$的类型。
...
形如\tcode{\{$e$\}}的表达式总是被视为一个块而不是结构字面量。\enternote 如果需要创建结构字面量，请显式写为\tcode{\{$e$:$e$\}}。\exitnote
...
\pnum
\tcode{$T$($a_1$, $\ldots$, $a_n$)}以参数给定的参数创建$T$的对象。创建对象时，会以这些参数调用给定的初始化器。以这种方式初始化的对象也可以带有lambda块。如果这个对象的初始化器只接受这一个参数，也可以省略括号。
...
\pnum
\tcode{$T$\{$k_1$:$v_1$, $\ldots$, $k_n$:$v_n$\}}以结构方式创建$T$的对象。
...
\begin{itemize}
    \item 以具名参数初始化，等价于\tcode{$T$($k_1$:$v_1$, $\ldots$, $k_n$:$v_n$)}；
    \item 以单个结构字面量的方式初始化，等价于\tcode{$T$(\{$k_1$:$v_1$, $\ldots$, $k_n$:$v_n$\})}。
\end{itemize}

\pnum
\tcode{$T$\{\}}、\tcode{$T$\{$e$\}}和\tcode{$T$[...$e$]}的语法歧义将按与普通字面量类似的规则解决（见上）。

\pnum
如果使用了内建的初始化方式或者采用了不抛出异常的初始化器，则结果类型为$T$；否则，结果类型为\tcode{$T$ throw $E$}，其中$E$为抛出的异常类型。
...
\pnum
成员访问运算符用于访问对象的成员。\tcode{$o$.$m$}表示对象$o$的成员$m$。如果$o$没有成员$m$，若它后跟一个函数调用运算符，则作为方法调用处理；否则这是一个编译错误。

\pnum
\tcode{$o$.$i$}用于元组成员的访问，表示$o$的第$i$个元素。$i$必须是一个不包含前缀或后缀的十进制字面量。如果$i$大于等于元组的长度，这是一个编译错误。

\pnum
\tcode{$o$.($e$)}首先对$e$求值。如果得到一个\tcode{symbol}类型的值，则对$o$进行成员访问。如果得到一个整数类型的值，则对$o$进行元组成员访问。此时$o$必须是一个常量表达式。否则，$o$必须是一个带有\tcode{this}参数的函数类型，且其必须后跟一个函数调用运算符，此时将视为对$e$调用方法$o$。
...
\pnum
后缀\tcode{!}将可空类型转换为非可空类型。对表达式而言，如果\tcode{$e$!}不为\tcode{nil}，则结果为$e$；否则\tcode{panic}。参见~\ref{except.panic}。
...
\pnum
如果$e$是结果类型\tcode{$T$ throw $E$}，则\tcode{$e$?}与\tcode{try $e$ catch \{ nil \} else let $v$ \{ some $v$ \}}等价。
...
对第一种情况，结果等于将两个字符串左右连接得到的结果；对第二种情况，\tcode{$x$ \~ $y$}等于\tcode{[$x$, ...$y$]}或\tcode{[...$x$, $y$]}，取决于哪个操作数是序列；对第三种情况，\tcode{$x$ \~ $y$}等于\tcode{[\dots$x$, ...$y$]}。
