...
\pnum
后缀\tcode{!}将可空类型转换为非可空类型。对表达式而言，如果$e\tcode{!}$不为\tcode{nil}，则结果为$e$；否则调用\tcode{panic}。参见~\ref{except.panic}。
...
\pnum
结果类型重载了空值检测运算符。参见~\ref{core.error.impl}。
...
\begin{bnf}{CastExpr}
    ...
    SuffixExpr \terminal{as} \terminal{?} Type \br
    SuffixExpr \terminal{as} \terminal{!} Type
\end{bnf}
